...
echo 	"<a href='modifierprofil.php'><li class='droite'><img src='image/profil1.png' height='30px'></li></a>
		...
		<input type='submit' value='Recherche'></form></li>
			...
			$resultat=$conn->query("SELECT * FROM utilisateur,chanson where chanson.fk_num=utilisateur.num and chanson.fk_idTheme=6");
			...
			while($donnee=$resultat->fetch())
			{
				...
				." ".$donnee['nom']."</i><br><input type='submit' name='likereggae[$id]' value=$likes required>"; // on utilise required pour empecher un like a chaque actualisation de la page
