...
echo 	"<a href='modifierprofil.php'><li class='droite'><img src='image/	profil1.png' height='30px'></li></a>
		...
		<input type='submit' value='Recherche'></form></li>
			...
			$resultat=$conn->query("SELECT * from utilisateur, chanson where chanson.fk_num=utilisateur.num order by nb_likes DESC limit 10");
			//affiche les 10 chansons les plus aimées.
			...
			while($donnee=$resultat->fetch())
			{
				...
				." ".$donnee['nom']."</i><br><input type='submit' name='likechansonpluslikee[$id]' value=$likes required></td></font>"; // on utilise required pour empecher un like a chaque actualisation de la page
				if($i%5==0){
					echo"</tr>";
				}		
